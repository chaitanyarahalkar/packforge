//! Native-corpus payload probe for the clean-room `PFG-HLZ/1` feasibility gate.
...
use packforge_core::{
    LINUX_X86_64_RUNTIME_V2_CODEC5, PackOptions, pack_executable_v2_codec5, pfg_hlz,
};
...
fn main() -> ExitCode {
    match run() {
        Ok(()) => ExitCode::SUCCESS,
        Err(message) => {
            eprintln!("m2_pfg_hlz_probe: {message}");
            ExitCode::from(1)
        }
    }
}
...
fn run() -> Result<(), String> {
    ...
    let payload = pfg_hlz::encode(&original).map_err(|error| error.to_string())?;
    if pfg_hlz::decode(&payload, original.len()).map_err(|error| error.to_string())? != original {
        return Err("PFG-HLZ round trip did not recover the original".to_owned());
    }
