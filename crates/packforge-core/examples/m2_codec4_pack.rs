...
use packforge_core::{PackOptions, Profile, pack_executable_v2_codec4};

fn main() {
    let mut arguments = env::args().skip(1);
    let input_path = arguments
        .next()
        .expect("usage: m2_codec4_pack INPUT LOADER OUTPUT");
    ...
    let mode = fs::metadata(input_path)
        ...
        .permissions()
        .mode();
    let packed = pack_executable_v2_codec4(
        &original,
        mode,
        PackOptions {
